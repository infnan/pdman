...
class Tab extends React.Component {
  ...
  getChildContext() {
    return {
      height:
        this.state.height ||
        document.body.clientHeight - defaultConfig.menuHeight - 20,
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
            {[].concat(children).map((child) => (
              ...
              >
                ...
                >
                  {child.props.realName || child.props.title}
                </span>
                {type === 'edit' ? (
                  <Icon
          ...
          <div
            ...
            style={{
              width: 25,
              cursor: 'pointer',
              display: tempTabs.length > 0 ? '' : 'none',
            }}
          >
          ...
          <div
            ...
            style={{
              display:
                this.state.showDropMenu && tempTabs.length > 0 ? '' : 'none',
            }}
          >
            <ul>
              {tempTabs.map((tab) => (
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div
          ...
          style={{
            height: `${
              (this.state.height ||
                document.body.clientHeight - defaultConfig.menuHeight - 20) + 10
            }px`,
            overflow: 'auto',
          }}
        >
          {[].concat(children).map((child) => (
            <div
              key={child.key}
              style={{
                display: child.key === this._getShowKey() ? '' : 'none',
                height: '100%',
              }}
            >
