...
export default class Header extends React.Component {
  ...
  _iconClick = (type) => {
    switch (type) {
      case 'minimize':
        ipcRenderer.sendSync('headerType', 'minimize');
        break;
      case 'restore':
        this.setState({
          flag: !this.state.flag,
        });
        ipcRenderer.sendSync('headerType', 'restore');
        break;
      case 'maximize':
        this.setState({
          flag: !this.state.flag,
        ...
        break;
      case 'close':
        ipcRenderer.sendSync('headerType', 'close');
        break;
      default:
        break;
    }
  ...
  render() {
    ...
    return (
      <div style={{ background: '#E3E3E5' }}>
        <div className='pdman-top-border' onDoubleClick={this._onDbClick}>
          {}
        </div>
        <div className='pdman-header'>
          <div className='pdman-header-left'>
            <div className='pdman-header-left-icon'>{}</div>
            ...
            <div className='pdman-header-left-app'>&nbsp;-&nbsp;PDMan</div>
          </div>
          <div className='pdman-header-right'>
            <Icon
              type='fa-window-minimize'
              onClick={() => this._iconClick('minimize')}
            />
            <Icon
            ...
            />
            <Icon
              type='fa-window-close-o'
              onClick={() => this._iconClick('close')}
            />
          </div>
        </div>
      </div>
