...
class ChangeCode extends React.Component {
  ...
  render() {
    const { width, again, synchronous, preSynchronous, flagSynchronous } =
      this.state;
    const {
      database = [],
      messages,
    ...
    const selectCode =
      code ||
      database.filter(db => db.defaultDatabase).map(db => db.code)[0];
    return (
      <div className='pdman-change-code'>
        <Icon
        ...
        <div
          ...
          style={{
            width: width === '50%' ? '50%' : '0px',
            display: width === '50%' ? '' : 'none',
          }}
        >
        ...
        <div className='pdman-change-code-context' style={{ width: width }}>
          <span className='pdman-change-code-context-header'>
            {version
              ? `变化脚本(${
                  compareStringVersion(version, dbVersion) <= 0
                    ? '已同步'
                    : '未同步'
                })`
              : '变化脚本'}
          ...
          <div>
            <div className='pdman-data-table-content-tab'>
              <div
                key={selectCode}
            ...
            <div className='pdman-data-tab-content'>
              ...
              <Button
                ...
                style={{
                  marginLeft: 10,
                  display:
                    version && compareStringVersion(version, dbVersion) > 0
                      ? ''
                      : 'none',
                }}
...
class CustomerVersionCheck extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dbVersion: {
        ...
        incrementVersion:
          (props.versions[0] && props.versions[0].version) || '',
      },
  ...
  _onCheck = () => {
    ...
    if (
      compareStringVersion(dbVersion.incrementVersion, dbVersion.initVersion) <=
      0
    ) {
      Modal.error({
        title: '比较失败',
        message: '增量脚本的版本号不能小于或等于初始版本的版本号',
      });
    } else {
      // 读取两个版本下的数据信息
  ...
  render() {
    const { versions, dbData, constructorMessage, defaultDB, dataSource } =
      this.props;
    ...
    return (
      <div>
        <div style={{ display: 'flex' }}>
          ...
          <Button style={{ height: 30 }} onClick={this._onCheck}>
            比较
          </Button>
        </div>
        <div>
...
class DatabaseVersionContext extends React.Component {
  ...
  render() {
    ...
    return (
      <div className='pdman-db-version-context'>
...
export default class DatabaseVersion extends React.Component {
  ...
  _getVersionMessage = () => {
    getDirListPromise(this.basePathDir)
      .then((res) => {
        // 从每个版本的文件中获取版本信息
        ...
        Promise.all(
          versions.map((v) => {
            return new Promise((resolve, reject) => {
              readFilePromise(`${this.basePathDir}${v}`)
                .then((data) => {
                  resolve({
                    ...data,
                    file: v,
    ...
    if (this.allMatchCount > 0) {
      if (this.count < this.allMatchCount) {
        ...
        this.code.scrollLeft =
          this.positions[this.count - 1].left - modalWidth / 2;
  ...
  _search = () => {
    ...
    if (this.searchValue) {
      ...
      this.allMatchCount =
        (this.tempHtml.match(regExp) && this.tempHtml.match(regExp).length) ||
        0;
      this.countDom.innerHTML = `${this.allMatchCount === 0 ? 0 : 1}/${
        this.allMatchCount
      }`;
      this.code.innerHTML = (this.tempHtml || '').replace(
        regExp,
        `<span class='pdman-select-value'>${this.searchValue}</span>`,
      );
    } else {
    ...
    const left = document.body.clientWidth * 0.1 + 16;
    const modalWidth = document.body.clientWidth * 0.8 - 32;
    //console.log(top);
    this.spans = Array.from(this.code.querySelectorAll('.pdman-select-value'));
    this.positions = this.spans.map((s) => {
  ...
  _connectJDBC = (selectJDBC, cb, cmd) => {
    ...
    const tempValue = value
      ? `${value}${this.split}bin${this.split}java`
      : 'java';
    let modal = null;
    const onOk = () => {
      modal && modal.close();
    ...
    if (selectJDBC.showModal) {
      modal = openModal(
        <Code
          style={{ height: 300 }}
          data={`执行同步命令：${tempValue} -Dfile.encoding=utf-8 -jar ${jar} ${cmd} ${this._getParam(
            selectJDBC,
          ).join(' ')}`}
        />,
        {
    ...
    const customerDriver = _object.get(
      selectJDBC,
      'properties.customer_driver',
      '',
    );
    const commend = [
    ...
    execFile(tempValue, commend, (error, stdout, stderr) => {
      modal && modal.close();
  ...
  _generateSQL = (
    ...
    fileExistPromise(tempPath, true, data, '.sql').then(() => {
      if (path) {
          ...
          (result) => {
            ...
            if (result.status === 'SUCCESS') {
              Modal.success({
                title: '数据库同步成功',
                message: (
                  <div onKeyDown={e => this._onKeyDown(e)}>
                    <div
                      style={{
                        height: '30px',
                        display: 'flex',
                    ...
                    <Code
                      ...
                      data={
                        cmd === 'updateVersion'
                          ? '标记成功！'
                          : this._getProperties(result.body || result)
                      }
                    />
                  </div>
                ),
  ...
  _checkVersionCount = (version) => {
    ...
    if (!dbVersion) {
      ...
      lowVersions = versions.filter(
        v => compareStringVersion(v.version, dbVersion) > 0,
      );
    }
    return lowVersions
          ...
          Modal.confirm({
            ...
            onOk: (m) => {
              ...
              let tempValue = {
                ...
                readDBType: 'read',
                sqlPath: '',
                ..._object.omit(configData.synchronous || {}, [
                  'readDBType',
                  'sqlPath',
                ]),
              };
              if (tempValue.readDBType !== 'read' && !tempValue.sqlPath) {
                ...
                this._generateSQL(
                  ...
                  data,
                  updateVersion,
                  tempValue.readDBType !== 'read' && tempValue.sqlPath,
                );
              }
            },
          });
  ...
  _saveNewVersion = (cb) => {
    ...
    getDirListPromise(this.basePathDir)
      .then((res) => {
        const versions = res
          .filter(r => r.endsWith('.pdman.json'))
          .map((r) => {
            const names = r.split('-');
        ...
        readFilePromise(checkPath).then((checkDataSource) => {
          ...
          currentTables.forEach((table) => {
            ...
            if (checkTableNames.includes(table.title)) {
              ...
              // 将两个表的所有的属性循环比较
              const checkFields = (checkTable.fields || []).filter(
                f => f.name,
              );
              ...
              // 1.2.2 其他信息
              const entityResult = this._compareEntity(
                ...
                onOk: (modal) => {
                  if (!tempValue.version || !tempValue.message) {
                    ...
                    compareStringVersion(
                      tempValue.version,
                      this.state.versions[0].version,
                    ) <= 0
                  ) {
                    Modal.error({
                      title: '操作失败',
                      message: '新版本不能小于或等于已经存在的版本',
                    });
                  } else {
      ...
      />,
      {
        ...
        onOk: (modal) => {
          ...
          if (!tempValue.version || !tempValue.message) {
            ...
            tempVersions.map(v => v.version).includes(tempValue.version)
          ) {
            Modal.error({
              title: '操作失败',
              message: '该版本号已经存在了',
  ...
  _updateVersionFile = (newVersion, oldVersion, status, cb) => {
    ...
    if (status === 'update') {
      ...
      // 创建新增的
      fileExistPromise(newVersionPath, true, _object.omit(newVersion, ['file']))
        .then(() => {
  ...
  render() {
    ...
    return (
      <div className='pdman-db-version'>
        <div className='pdman-db-version-opt-container'>
          <div className='pdman-db-version-opt'>
            ...
            >
              ...
              <span className='pdman-db-version-opt-synchronous-name'>
                同步配置
              </span>
            </div>
            <div
            ...
            >
              ...
              <span className='pdman-db-version-opt-rebuild-name'>
                重建基线
              </span>
            </div>
            <div
            ...
            >
              ...
              <span className='pdman-db-version-opt-compare-name'>
                任意版本比较
              </span>
            </div>
          </div>
        ...
        {versionData ? (
          ...
          <div className='pdman-db-version-list'>
            <span style={{ textAlign: 'center', color: '#2492E6' }}>
              {currentDB
                ? `当前使用的数据库：【${currentDB}】,
              当前数据库版本：【${
                dbVersion === 'v0.0.0'
            ...
            >
              <div className='pdman-db-version-list-item'>
                <div className='pdman-db-version-list-item-time'>{}</div>
                <div className='pdman-db-version-list-item-line'>
                  <div>{}</div>
                  <span
                    className={`pdman-db-version-list-item-line-${
                      changes.length > 0
                        ? 'current-changes'
                        : 'current-unchange'
                    }`}
                  ...
                  <span
                    ...
                    onClick={() =>
                      this._showChanges(
                        ...
                        false,
                        this.props.dataSource,
                        versions[0] || this.props.dataSource,
                      )
                    }
                  >
                    {changes.length > 0
                      ? '当前内容与上一版本的内容有变化，但未保存版本！'
                      : '当前内容与上一版本内容无变化'}
                  </span>
                </div>
              </div>
                ...
                <div className='pdman-db-version-list-item'>
                  <div className='pdman-db-version-list-item-time'>
                    <span className='pdman-db-version-list-item-time-all'>
                      <span className='pdman-db-version-list-item-time-day'>
                        {v.date.split(' ')[0]}
                      </span>
                      <span className='pdman-db-version-list-item-time-min'>
                        {v.date.split(' ')[1]}
                      </span>
                    </span>
                  </div>
                  ...
                  <div className='pdman-db-version-list-item-tag'>
                    ...
                    <div
                      ...
                      className={`pdman-db-version-list-item-tag-${
                        compareStringVersion(v.version, dbVersion) <= 0
                          ? 'success'
                          : 'error'
                      }`}
