...
export default class ExportSQL extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      data: getAllDataSQLByFilter(props.dataSource, props.defaultDb, [
        'deleteTable',
        'createTable',
        'createIndex',
        'updateComment',
  ...
  getValue = () => {
    ...
    if (this.state.export === 'all') {
      ...
      tempValue = Object.keys(this.state.type).filter(
        t => this.state.type[t],
      );
    }
    return {
  ...
  _selectTable = () => {
    ...
    openModal(
      <TreeSelect
        data={dataSource.modules || []}
        defaultSelecteds={selectTable}
      />,
      {
        title: '导出数据表选择',
        onOk: (m, c) => {
  ...
  _execSql = () => {
    ...
    const dbData = _object
      .get(dataSource, 'profile.dbs', [])
      .filter(d => d.defaultDB)[0];
    ...
    if (dbData) {
      ...
      fileExistPromise(tempPath, true, this.state.data, '.sql').then(() => {
        if (dbData) {
          this._connectJDBC(
            {
              ...dbData,
      ...
      Modal.error({
        title: '执行失败',
        message:
          '未配置默认数据库，无法执行SQL，请到数据库版本界面配置默认数据库！',
      });
    }
  };
  render() {
      ...
      >
        ...
        >
          ...
          >
            ...
            <span style={{ marginLeft: 10 }}>
              {selectTable === null
                ? '默认导出所有数据表'
                : `当前已选择数据表数量：${
                    selectTable.filter(k => k.includes('/')).length
                  }`}
            </span>
            <Button
              style={{ marginLeft: 10 }}
          ...
          >
            ...
            >
              <Radio
                wrapperStyle={{ width: 20, marginLeft: 10 }}
                value='customer'
              >
                自定义
              </Radio>
              <Radio wrapperStyle={{ width: 20, marginLeft: 10 }} value='all'>
                全部
              </Radio>
            </RadioGroup>
          </div>
          ...
          >
            <Button onClick={this._preview}>预览</Button>
          </div>
        </div>
        <div style={{ border: 'solid 1px #DFDFDF' }}>
          <div style={{ margin: '10px 0px' }}>
            <Button type='primary' onClick={this._export}>
              导出
            </Button>
            <Button
              style={{ marginLeft: 10 }}
