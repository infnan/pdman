...
export default class Setting extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      defaultFieldsType: _object.get(
        props.dataSource,
        'profile.defaultFieldsType',
        '1',
      ),
    };
    this.inputInstance = [];
    ...
    this.wordTemplateConfig = _object.get(
      props.dataSource,
      'profile.wordTemplateConfig',
      '',
    );
  ...
  _getStyle = (code) => {
    ...
    if (code === 'remark') {
      ...
      width =
        code !== 'pk' && code !== 'notNull' && code !== 'autoIncrement'
          ? '100%'
          : 'auto';
    }
    return {
      ...
      >
        <div className={`${prefix}-data-table-content-tab`}>
          ...
          <div
            ...
            className={`${prefix}-data-table-content-tab${
              this.state.tabShow === 'java' ? '-selected' : '-unselected'
            }`}
          >
            JAVA环境配置
          </div>
          <div
            ...
            className={`${prefix}-data-table-content-tab${
              this.state.tabShow === 'SQL' ? '-selected' : '-unselected'
            }`}
          >
            SQL分隔符配置
          </div>
          <div
            ...
            className={`${prefix}-data-table-content-tab${
              this.state.tabShow === 'word' ? '-selected' : '-unselected'
            }`}
          >
            WORD模板配置
          </div>
        ...
        <div className={`${prefix}-data-tab-content`}>
          <div
            style={{
              width: '100%',
              display: this.state.tabShow === 'fields' ? '' : 'none',
            }}
          >
            <div className={`${prefix}-data-table-content-table-opt-icon`}>
              <Com.Icon
                onClick={() =>
                  selectedTrs.length !== 0 && this._moveField('up')
                }
                className={
                          ...
                          return (
                            ...
                            >
                              <ThCom
                                ...
                                type={
                                  field[column.code] ? 'fa-eye-slash' : 'fa-eye'
                                }
                                onClick={(e) =>
                                  this._relationNoShowClick(
                                    e,
                          ...
                          <th key={`${column.code}-${field.key}`}>
                            {
                              <ThCom
                                ...
                                ref={(instance) => {
                                  if (column.com === 'Input') {
                                    ...
                                    this.inputInstance[index][rowIndex] =
                                      instance;
                                  }
                                }}
                                onFocus={() =>
                                  column.com === 'Input' &&
                                  this._onFocus(index, rowIndex)
                                }
                                onChange={(e) =>
                                  this._inputOnChange(e, field.key, column.code)
                                }
                              ...
                              >
                                {column.com === 'Select' &&
                                  this._getOptions(dataTypes, column.code)}
                              </ThCom>
                            }
                          </th>
                        );
          ...
          <div
            style={{
              width: '100%',
              display: this.state.tabShow === 'java' ? '' : 'none',
            }}
          >
            <JavaHomeConfig
              onChange={this._javaHomeChange}
              data={this.javaConfig}
              project={project}
            />
          </div>
          <div
            style={{
              width: '100%',
              display: this.state.tabShow === 'SQL' ? '' : 'none',
            }}
          >
            <SQLConfig
              onChange={this._sqlSeparatorChange}
              data={this.sqlConfig}
            />
          </div>
          <div
            style={{
              width: '100%',
              display: this.state.tabShow === 'word' ? '' : 'none',
            }}
          >
            <WORDConfig
              onChange={this._wordTemplateChange}
              data={this.wordTemplateConfig}
            />
          </div>
