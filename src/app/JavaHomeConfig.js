...
export default class JavaHomeConfig extends React.Component {
  constructor(props) {
    ...
    this.state = {
      data: {
        JAVA_HOME:
          props.data.JAVA_HOME ||
          process.env.JAVA_HOME ||
          process.env.JER_HOME ||
          '',
  ...
  _openDir = (callBack, type) => {
    const properties =
      type === 'JAVA_HOME' ? 'openDirectory' : 'multiSelections';
    const filter =
      type === 'JAVA_HOME' ? [] : [{ name: 'jar', extensions: ['jar'] }];
  ...
  _connectJDBC = () => {
    ...
    const tempValue = value
      ? `${value}${this.split}bin${this.split}java`
      : 'java';
    execFile(tempValue, ['-jar', jar], (error, stdout, stderr) => {
      if (error) {
  ...
  render() {
    const { data = {} } = this.state;
    return (
      <div className='pdman-config-java'>
        <div className='pdman-config-java-config'>
          ...
          <div className='pdman-config-java-config-button'>
            <Button
              onClick={() => this._iconClick('JAVA_HOME')}
              title='点击选择JAVA_HOME'
            >
              ...
            </Button>
            <Button style={{ marginLeft: 5 }} onClick={this._execFile}>
              测试
            </Button>
          </div>
        </div>
