...
export default class JDBCConfig extends React.Component {
  ...
  _trClick = (e, key) => {
    ...
    if (tempSelectedTrs.some(tr => tr === key)) {
      tempSelectedTrs =
        e && e.shiftKey ? tempSelectedTrs.filter(tr => tr !== key) : [];
    } else {
  ...
  _deleteDB = () => {
    ...
    this.setState(
      ...
      () => {
        onChange &&
          onChange(
            this.state.data.map(field => _object.omit(field, ['key'])),
          );
      },
    );
  };
  _addDB = () => {
    ...
    const selectedTrsIndex = tempFields
      ...
      .filter(field => field !== null);
    const database =
      _object.get(dataSource, 'dataTypeDomains.database', [])[0] || {};
  ...
  _getJAVAVersion = (java, cb) => {
    ...
    execFile(java, ['-version'], (error, stdout, stderr) => {
      if (error) {
        ...
        // 2.获取版本号的第一，第二位
        const currentVersion = (versionNumber.split('.') || []).map(v =>
          v.replace('"', ''),
        );
        let flag = false;
        ...
        if (!flag) {
          ...
          execFile(tempValue, commend, (error, stdout, stderr) => {
            ...
            if (tempResult.status !== 'SUCCESS') {
              Modal.error({
                title: '连接失败',
                message: tempResult.body || tempResult,
              });
            } else {
              Modal.success({
                title: '连接成功',
                message: `${tempResult.body}!数据库连接设置配置正确`,
              });
            }
          });
        }
      ...
      <div>
        <div style={{ border: 'solid 1px green', padding: 5, margin: 5 }}>
          ...
          <div style={{ color: 'green' }}>
            driver_class：
            <span style={{ color: 'red', userSelect: 'text' }}>
              {mysql.driverClass}
            </span>
          </div>
          <div style={{ color: 'green' }}>
  ...
  render() {
    ...
    return (
      <div className='pdman-jdbc-config'>
            ...
            {data.map((d, index) => (
              <div
                className={`pdman-jdbc-config-left-db-list-item pdman-data-table-content-table-normal-tr
                        ${
                          selectedTrs.some(tr => tr === d.key)
                            ? 'pdman-data-table-content-table-selected-tr'
                            : ''
                        }`}
                key={d.key}
              >
                ...
                >
                  ...
                  >
                    <span className='pdman-jdbc-config-left-db-list-item-index'>
                      {index + 1}
                    </span>
                    <Input
                      onChange={e => this._onDBChange(d.key, e, 'name')}
                      value={d.name}
                    />
                    <Select
                      onChange={e => this._onDBChange(d.key, e, 'type')}
                      defaultValue={d.type}
                    >
                      {database.map(db => (
        ...
        <div
          className='pdman-jdbc-config-right'
          style={{ display: selectedTrs.length > 0 ? '' : 'none' }}
        >
          <div className='pdman-jdbc-config-right-com'>
            <div className='pdman-jdbc-config-right-com-label'>
              <span title='目前mysql,sqlserver,oracle,postgresql无需配置'>
                自定义驱动:
              </span>
            </div>
          ...
          <div className='pdman-jdbc-config-right-com'>
            <div className='pdman-jdbc-config-right-com-label'>
              <span>
                ...
                <span className='pdman-jdbc-config-right-com-label-require'>
                  driver-class:
                </span>
              </span>
            </div>
