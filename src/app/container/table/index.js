...
class DataTable extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.dataSource !== nextProps.dataSource) {
      ...
      const newTableData = this._initTableData(
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    return (
      ...
      nextProps.dataSource.dataTypeDomains !==
        this.props.dataSource.dataTypeDomains
    );
  }
  ...
  save = (cb) => {
    ...
    if (title !== table && tables.includes(title)) {
      ...
      if (cb) {
        ...
        Modal.error({
          title: '保存失败',
          message: `数据表【${this.currentTable}】代码不能为空`,
          width: 300,
        });
      }
    } else {
  ...
  _deleteKey = (dataTable) => {
    const tempFields = (dataTable.fields || []).map(field =>
      _object.omit(field, ['key']),
    );
    return {
      ...dataTable,
      fields: tempFields,
      indexs: (dataTable.indexs || [])
        .map((field) => {
          return {
            ..._object.omit(field, ['key']),
            fields: (field.fields || []).filter(f =>
              tempFields.map(fd => fd.name).includes(f),
            ),
          };
        })
  ...
  _initTableData = (dataSource, module, table) => {
    const moduleData = {
      ...((dataSource.modules || []).filter(mo => mo.name === module)[0] ||
        {}),
    };
    const tableData = {
      ...((moduleData.entities || []).filter(
        entity => entity.title === table,
      )[0] || {}),
    };
    const fields = (tableData.fields || []).map(field => ({
      ...field,
      key: `${uuid()}-${field.name}`,
    }));
    const indexs = (tableData.indexs || []).map(field => ({
      ...field,
      key: `${uuid()}-${field.name}`,
    }));
  ...
  _getAllTableData = () => {
    const { dataTable } = this.state;
    const summary =
      (this.tableSummaryInstance &&
        this.tableSummaryInstance.getSummaryData()) ||
      {};
    const table = this._getDataTable();
    const indexsDate =
      (this.indexConfigInstance && this.indexConfigInstance.getData()) ||
      dataTable.indexs ||
      [];
    return {
      ...dataTable,
  ...
  render() {
    ...
    return (
      <div className={`${prefix}-data-table`}>
        ...
        <div className={`${prefix}-data-table-content`}>
          <div className={`${prefix}-data-table-content-tab`}>
            <div
              ...
              className={`${prefix}-data-table-content-tab${
                this.state.tabShow === 'summary' ? '-selected' : '-unselected'
              }`}
            >
              基本信息
            </div>
            <div
              ...
              className={`${prefix}-data-table-content-tab${
                this.state.tabShow === 'fields' ? '-selected' : '-unselected'
              }`}
            >
              字段信息
            </div>
            <div
              ...
              className={`${prefix}-data-table-content-tab${
                this.state.tabShow === 'codes' ? '-selected' : '-unselected'
              }`}
            >
              代码信息
            </div>
            <div
              ...
              className={`${prefix}-data-table-content-tab${
                this.state.tabShow === 'indexs' ? '-selected' : '-unselected'
              }`}
            >
              索引信息
            </div>
          ...
          <div style={{ height: height - 145, overflow: 'auto' }}>
            ...
            <div
              style={{ display: this.state.tabShow === 'fields' ? '' : 'none' }}
            >
              <Table
                height={height}
            ...
            <div
              style={{ display: this.state.tabShow === 'codes' ? '' : 'none' }}
            >
              <TableDataCode
            ...
            <div
              style={{
                display: this.state.tabShow === 'indexs' ? '' : 'none',
                height: '100%',
              }}
            >
              <TableIndexConfig
