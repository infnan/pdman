...
export default class TableCell extends React.Component {
  componentWillReceiveProps(nextProps) {
    ...
    if (
      com === 'Input' &&
      (index !== this.props.index || rowIndex !== this.props.rowIndex)
    ) {
  ...
  shouldComponentUpdate(nextProps) {
    ...
    return (
      nextProps.field[nextProps.column.code] !==
        this.props.field[this.props.column.code] ||
      nextProps.dataTypes !== this.props.dataTypes ||
      (nextProps.column.code === 'dataType' &&
        nextProps.field.type !== this.props.field.type)
    );
  }
  ...
  _getDefaultDataType = (type) => {
    ...
    const dbCode =
      (defaultDatabase && defaultDatabase.code) ||
      (database[0] && database[0].code) ||
      '';
    if (dbCode) {
  ...
  render() {
    const { column, field, ThCom, index, rowIndex, setInputInstance } =
      this.props;
    return (
          ...
          <ThCom
            suffix={
              column.code === 'remark' ? (
                <span
                  onClick={() =>
                    this._openRemark(field[column.code], field.key)
                  }
                >
                  ...
                </span>
            ...
            value={
              (column.code === 'dataType'
                ? this._getDefaultDataType(field.type)
                : field[column.code]) || ''
            }
            style={{
              height:
