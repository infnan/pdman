...
export default class TableDataCode extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      nextState.changes !== this.state.changes ||
  ...
  _getTableCode = (code, templateShow) => {
    ...
    let tempChanges = changes.filter((c) => {
      ...
      return (
        (templateShow === 'createFieldTemplate' &&
          c.type === 'field' &&
          c.opt === 'add' &&
            ...
            database.map(db => (
              <div
                ...
                className={`${prefix}-data-table-content-tab${
                  currentCode === db.code ? '-selected' : '-unselected'
                }`}
              >
                {db.code}
              </div>
