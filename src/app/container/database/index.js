...
class Database extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.defaultTemplates = _object.get(
      defaultData,
  ...
  _openHelp = (tabShow) => {
    ...
    const onOk = () => {
      ...
      this.setState({
        value: {
          ...value,
          [tabShow]:
            (templateHelp && templateHelp.getTemplate()) ||
            value[tabShow] ||
            '',
        },
      });
    ...
    const onAllOk = () => {
      modal && modal.close();
      const { value = {} } = this.state;
      const currentTemplate = this.defaultTemplates.filter(
        (t) =>
          t.code.toLocaleLowerCase() ===
          templateHelp.getCurrentDb().toLocaleLowerCase()
      )[0];
      this.setState({
        value: {
          ...value,
    ...
    modal = openModal(<TemplateHelp didMount={instance} />, {
      autoFocus: true,
      footer: [
        <Button key='onAllOk' onClick={onAllOk} type='primary'>
          使用此数据库案例模板
        </Button>,
        <Button
          key='ok'
        ...
        <Button key='cancel' onClick={onCancel} style={{ marginLeft: 10 }}>
          关闭
        </Button>,
      ],
      title: '默认模板配置案例',
  ...
  _useAllDefaultTemplate = () => {
    ...
    const currentTemplate = this.defaultTemplates.filter(
      (t) => t.code.toLocaleLowerCase() === code.toLocaleLowerCase()
    )[0];
    if (!currentTemplate) {
      Modal.error({
        title: '使用默认模板失败',
        message:
          '无匹配数据库，请点击[从默认模板中选取]选择需要使用的数据库模板',
      });
    } else {
      this.setState((state) => {
        return {
  ...
  render() {
    ...
    const mode = this._getMode(
      _object.get(value, 'code', '').toLocaleLowerCase()
    );
    return (
      <div
                    ...
                    <span
                      onClick={() =>
                        this._previewEdit(
                          this.state.tabShow,
                          mode,
                          _object.get(value, this.state.tabShow, '')
                        )
                      }
                      style={{
                        cursor: 'pointer',
