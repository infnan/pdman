...
export default class Parse extends React.Component {
  ...
  getSelectedEntity = (cb) => {
    ...
    if (this.state.keys.some(k => exists.includes(k.title))) {
      Modal.confirm({
        title: '温馨提示',
        message:
          '勾选的数据表中包含模型中已经存在的数据表，继续操作将会覆盖模型中的数据，是否继续？',
        onOk: (m) => {
          m && m.close();
  ...
  checkField = (data) => {
    ...
    // 从数据库解析中获取到的数据表
    const entities = _object
      .get(data, 'module.entities', [])
      .map(d => d.title);
    entities.forEach((e) => {
            ...
            <span style={{ color: 'green' }}>
              ...
              {status === 'SUCCESS' ? (
                <span>
                  ...
                  】张数据表， 当前模型中已经存在的有【
                  <span style={{ color: 'red' }}>{exists.length}</span>
                  】张表，请勾选需要添加到模型中的数据表！
                </span>
              ) : (
                '失败'
