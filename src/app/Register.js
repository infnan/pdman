        ...
        (error, stdout, stderr) => {
          ...
          if (tempResult.status === 'SUCCESS') {
            ...
            this.setState(
              ...
              () => {
                writeFile(
                  this.path + this.split + this.file,
                  this.state.register.code,
                )
                  .then(() => {
        ...
        {code ? (
          <div className='pdman-register-message'>
            <div className='pdman-register-message-item'>
              <span className='pdman-register-message-item-code-label'>
                注册码：
              </span>
              <span className='pdman-register-message-item-code'>{code}</span>
            </div>
            <div className='pdman-register-message-item'>
              <span className='pdman-register-message-item-status-label'>
                激活状态：
              </span>
              <span
                className={`pdman-register-message-item-status-${
                  status ? 'activation' : 'unactivation'
                }`}
              >
                {status ? '已激活' : '未激活'}
              </span>
            ...
            <div className='pdman-register-message-opt'>
              <Button icon='fa-refresh' onClick={this._clearRegister}>
                重新生成注册码
              </Button>
              <Button
                icon='fa-check'
          ...
          <div className='pdman-register-form'>
            <div className='pdman-register-form-item'>
              <span className='pdman-register-form-item-label pdman-register-form-item-label-require'>
                使用时长(月)：
              </span>
              <RadioGroup
              ...
              >
                <Radio wrapperStyle={{ width: '20px' }} value={1}>
                  <span>1个月</span>
                </Radio>
                <Radio wrapperStyle={{ width: '20px' }} value={3}>
                  <span>3个月</span>
                </Radio>
                <Radio wrapperStyle={{ width: '20px' }} value={12}>
                  <span>12个月</span>
                </Radio>
              </RadioGroup>
            </div>
            <div className='pdman-register-form-item'>
              ...
              <Input
                onChange={e =>
                  this._valueChange('mobilePhone', e.target.value)
                }
              />
            </div>
            <div className='pdman-register-form-item'>
              ...
              <Input
                onChange={e => this._valueChange('userName', e.target.value)}
              />
            </div>
            <div className='pdman-register-form-item'>
              ...
              <Input
                style={{ width: 400 }}
                onChange={e =>
                  this._valueChange('userCompany', e.target.value)
                }
              />
            </div>
            <div className='pdman-register-form-item'>
              ...
              <Input
                onChange={e =>
                  this._valueChange('companyAddress', e.target.value)
                }
              />
            </div>
