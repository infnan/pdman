...
const menus = [
  {
    ...
    key: 'new',
    icon: (
      <Icon type='addfolder' style={{ color: '#008000', marginRight: 5 }} />
    ),
  },
  {
    name: '重命名',
    key: 'rename',
    icon: <Icon type='fa-undo' style={{ color: '#F96B36', marginRight: 5 }} />,
  },
  {
    name: '删除',
    key: 'delete',
    icon: <Icon type='delete' style={{ color: '#FF0000', marginRight: 5 }} />,
  },
  {
    name: '复制',
    key: 'copy',
    icon: <Icon type='copy1' style={{ color: '#0078D7', marginRight: 5 }} />,
  },
  {
    name: '剪切',
    key: 'cut',
    icon: <Icon type='fa-cut' style={{ color: '#D2B3AF', marginRight: 5 }} />,
  },
  {
    name: '粘贴',
    key: 'paste',
    icon: <Icon type='fa-paste' style={{ color: '#6968E1', marginRight: 5 }} />,
  },
  {
    name: '打开',
    key: 'open',
    icon: (
      <Icon type='folderopen' style={{ color: '#C3D6E8', marginRight: 5 }} />
    ),
  },
];
export default class App extends React.Component {
  ...
  _getTabToFoldingTabs = () => {
    ...
    const tempTabs = tabs
      .filter((tab) => tab.key !== show)
      .filter((tab) => !tab.folding);
  ...
  _setting = () => {
    const { columnOrder, dataSource, project, register, updateRegister } =
      this.props;
    openModal(
      <Setting
    ...
    if (type === 'Markdown') {
      ...
      openDir((dir) => {
        ...
        saveImage(
          ...
          (images) => {
            Promise.all(
              Object.keys(images).map((mo) => {
                const base64Data = images[mo].replace(
                  /^data:image\/\w+;base64,/,
                  ''
                );
          ...
          saveImage(
            ...
            (images) => {
              ...
              Promise.all(
                ...
                const templatePath =
                  _object.get(dataSource, 'profile.wordTemplateConfig') ||
                  defaultPath;
                generateByJar(
                  dataSource,
                  {
                  ...
                  (error, stdout, stderr) => {
                    ...
                    if (tempResult.status !== 'SUCCESS') {
                      Modal.error({
                        title: `${type}导出失败!请重试！`,
                        message: `出错原因：${
                          tempResult.body || tempResult
                        }，可前往\${user.home}/logs/pdman目录查看出错日志`,
                      });
                    } else {
                      Modal.success({
                        title: `${type}导出成功！`,
        ...
        saveImage(
          ...
          (images) => {
            generateHtml(dataSource, images, projectName, (dataSource) => {
              writeFile(`${dir}/${projectName}.html`, dataSource)
                .then(() => {
                  modal && modal.close();
                  btn && btn.setLoading(false);
                  Message.success({
                    title: `html导出成功！导出目录：[${dir}]`,
                  });
                })
                .catch(() => {
                  modal && modal.close();
      ...
      const defaultDb = (database.filter((db) => db.defaultDatabase)[0] || {})
        .code;
      let modal = null;
      const onOk = () => {
        ...
        if (value.length === 0) {
          ...
          dialog.showSaveDialog(
            {
              title: '保存SQL文件',
              filters: [{ name: 'PDMan', extensions: ['sql'] }],
            },
            (file) => {
              if (file) {
  ...
  _export = () => {
    ...
    openModal(
      <div style={{ textAlign: 'center', padding: 10 }}>
        <Button icon='HTML' onClick={(btn) => this._exportFile('Html', btn)}>
          导出HTML
        </Button>
        <Button
          icon='wordfile1'
          style={{ marginLeft: 40 }}
          onClick={(btn) => this._exportFile('Word', btn)}
        >
          导出WORD
        </Button>
        <Button
          icon='pdffile1'
          style={{ marginLeft: 40 }}
          onClick={(btn) => this._exportFile('PDF', btn)}
        >
          导出PDF
        </Button>
        <Button
          icon='file1'
          style={{ marginLeft: 40 }}
          onClick={(btn) => this._exportFile('Markdown', btn)}
        >
          导出MARKDOWN
        </Button>
      </div>,
  ...
  _readDB = () => {
    ...
    const success = (keys, data) => {
      if (keys.length > 0) {
        ...
        let currentDataTypes = _object.get(
          dataSource,
          'dataTypeDomains.datatype',
          []
        );
        const database = _object.get(
          dataSource,
          'dataTypeDomains.database',
          []
        );
  ...
  _saveRelation = () => {
    const { show } = this.state;
    this.relationInstance[show] &&
      this.relationInstance[show].saveData(() => {
        //Modal.success({title: '保存成功', message: '保存成功', width: 200})
        Message.success({ title: '保存成功' });
      });
  };
  ...
  _checkShowTab = (tabs = [], show) => {
    ...
    if (tabs.length > 0) {
      ...
      return this._checkShowTab(
        tabs,
        tabs[showTabIndex === 0 ? 0 : showTabIndex - 1].key
      );
    }
    return show;
  };
  _tabClose = (value) => {
    ...
    if (tempValue.length > 0) {
      ...
      this.setState(
        {
          tabs: result.tabs,
          show: this._checkShowTab(
            result.tabs,
            (show === value && result.show && result.show.key) || show
          ),
  ...
  _getTabRealName = (value) => {
    ...
    const moduleData = (dataSource.modules || []).filter(
      (m) => m.name === module
    )[0];
    let entityData = null;
    if (moduleData) {
      entityData = (moduleData.entities || []).filter(
        (e) => e.title === entity
      )[0];
    }
    if (entityData) {
  ...
  _onDoubleClick = (value) => {
    ...
    if (value.startsWith('datatype&data&')) {
      DataTypeUtils.renameDataType(
        value.split('datatype&data&')[1],
        dataSource,
        (data) => {
      ...
      DatabaseUtils.renameDatabase(
        value.split('database&data&')[1],
        dataSource,
        (data) => {
          saveProject(`${project}.pdman.json`, data);
        }
      ...
      if (tempValue.value) {
        ...
        if (
          !tempTabs.some(
            (tab) => tab.key === `${tempValue.value}/${tempValue.icon}`
          )
        ) {
          tempTabs.push({
  ...
  _onContextMenu = (e, value, checked) => {
    ...
    if (checked.length > 1) {
      contextMenus = [
        {
          name: (
            <span>
              <Icon type='copy1' style={{ color: '#0078D7', marginRight: 5 }} />
              复制
            </span>
          ),
      ...
      if (value.startsWith('module&')) {
        contextMenus = contextMenus.concat(
          menus
            .map((menu) => {
              return {
                ...menu,
        ...
        contextMenus = contextMenus.concat(
          menus
            ...
            .filter(
              (menu) =>
                !menu.key.startsWith('open&') &&
                !menu.key.startsWith('delete&') &&
                !menu.key.startsWith('rename&')
            )
        );
        ...
        contextMenus = contextMenus.concat(
          menus
            ...
            .filter(
              (menu) =>
                !menu.key.startsWith('open&') && !menu.key.startsWith('rename&')
            )
        );
  ...
  _contextClick = (e, key, menu) => {
    ...
    if (keyArray[1]) {
      ...
      } else if (keyArray[1] === 'table' || keyArray[1] === 'entity') {
            ...
            tableUtils.deleteTable(module, table, dataSource, (data) => {
              saveProject(`${project}.pdman.json`, data, () => {
                ...
                if (
                  tabs
                    .map((tab) => tab.key)
                    .includes(`${module}&${table}/fa-table`)
                ) {
            ...
            this.setState({
              tabs: tabs.map((tab) => {
                ...
                if (module === oldModule) {
                  ...
                  const newKey =
                    type === 'entity'
                      ? `${newModule}&${table}/fa-table`
                      : `${newModule}/关系图/fa-wpforms`;
                  if (tempShow === tab.key) {
                    tempShow = newKey;
                  }
                  return {
  ...
  _onDrop = (drop, drag) => {
    ...
    if (dropType !== dragType) {
      ...
      const { dataSource, project, saveProject } = this.props;
      const datatype = _object.get(
        dataSource,
        `dataTypeDomains.${dropType}`,
        []
      );
  ...
  _onDataTableDrop = (drop, drag) => {
    if (drag.split('&')[0] === 'module') {
      ...
      if (drop.split('&')[0] !== 'module') {
        ...
        const dragIndex = (dataSource.modules || []).findIndex(
          (mo) => mo.name === dragModule
        );
        const dropIndex = (dataSource.modules || []).findIndex(
          (mo) => mo.name === dropModule
        );
        saveProject(`${project}.pdman.json`, {
          ...dataSource,
          modules: moveArrayPosition(
            dataSource.modules || [],
            dragIndex,
            dropIndex
          ),
...
// Modal.error({title: '移动失败', message: '无法跨模块移动数据表！', width: 200});
        ...
        const { saveProject, dataSource, project } = this.props;
        // 获取将要移动的数据表
        const dragModuleData = (dataSource.modules || []).filter(
          (module) => module.name === dragModule
        )[0];
        const dragEntityData = (dragModuleData.entities || []).filter(
          (entity) => entity.title === dragEntity
        )[0];
        // 更新关系图中的节点信息
        // 1.获取所有的关系图节点界面
        Object.keys(this.relationInstance).forEach((r) => {
          ...
          if (relationModuleName === dragModule) {
            ...
            if (this.relationInstance[r]) {
              ...
              // 设置新的节点
              this.relationInstance[r].setNodes(
                nodes.map((n) => {
          ...
          `${project}.pdman.json`,
          {
            ...
            modules: (dataSource.modules || []).map((module) => {
              if (module.name === dragModule) {
                ...
                const dragIndex = tempEntities.findIndex(
                  (entity) => entity.title === dragEntity
                );
                ...
                const tempEntities = [...(module.entities || [])];
                const dropIndex = tempEntities.findIndex(
                  (entity) => entity.title === dropEntity
                );
        ...
        saveProject(`${project}.pdman.json`, {
          ...
          modules: (dataSource.modules || []).map((module) => {
            if (module.name === dragModule) {
              ...
              const dragIndex = entities.findIndex(
                (entity) => entity.title === dragEntity
              );
              const dropIndex = entities.findIndex(
                (entity) => entity.title === dropEntity
              );
              return {
                ...module,
  ...
  _onMouseDown = (event) => {
    ...
    document.onmousemove = (e) => {
      if (!this.moveUp) {
        this.leftTabInstanceDom.style.width = `${
          offWidth - (downX - e.clientX)
        }px`;
        this.instanceDom.style.width = `calc(100% - ${
          offWidth - (downX - e.clientX)
        }px)`;
  ...
  _relationSearch = (e) => {
    const { show } = this.state;
    this.relationInstance[show] &&
      this.relationInstance[show].searchNodes(e.target.value);
  };
  _exportImage = () => {
    openModal(<ExportImg />, {
      title: '选择图片导出类型',
        ...
        dialog.showSaveDialog(
          ...
          (file) => {
            if (file) {
              const { show } = this.state;
              this.relationInstance[show] &&
                this.relationInstance[show].exportImg(file, type, () => {
    ...
    return (
      ...
      >
        <div className='pdman-home-header'>
          ...
          <div
            className='pdman-home-header-menu'
          ...
          >
              ...
              <div className='options-wrapper'>
                <div>
                  <ul className='other-options-menu'>
                   ...
                   ${
                     tools === 'file' || tools === 'entity'
                       ? 'menu-tools-edit-active'
                       : 'tools-content-enable-click'
                   }`}
                    ...
                    <li
                      className={`other-options-menu-tools
                   ${
                     tabs.length > 0 && tools === 'map'
                       ? 'menu-tools-edit-active'
                       : ''
                   }
                  ${
                    tabs.length > 0 && toolsClickable === 'map'
                      ? ''
                      : 'tools-content-un-click'
                  }`}
                      onClick={() =>
                        tabs.length > 0 &&
                        toolsClickable === 'map' &&
            ...
            >
              <div
                className='tools-content-tab'
                style={{
                  display: tools === 'file' || tools === 'entity' ? '' : 'none',
                }}
              >
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                    ...
                    >
                      <Icon
                        type='folderopen'
                        style={{ marginRight: 5, color: '#FFCA28' }}
                      />
                      打开
                    </div>
                    <div
                    ...
                    >
                      <Icon
                        type='addfolder'
                        style={{ marginRight: 5, color: '#96C080' }}
                      />
                      新建
                    </div>
                    <div
                    ...
                    >
                      <Icon
                        type='fa-save'
                        style={{ marginRight: 5, color: '#9291CD' }}
                      />
                      另存为
                    </div>
                  </div>
                  <div className='tools-content-group-name'>项目</div>
                </div>
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                    <div
                    ...
                    >
                      <Icon type='fa-link' style={{ marginRight: 5 }} />
                      数据库连接
                    </div>
                  </div>
                  <div className='tools-content-group-name'>配置</div>
                </div>
              </div>
              <div
                className='tools-content-tab'
                style={{
                  display: tabs.length > 0 && tools === 'map' ? '' : 'none',
                }}
              >
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                  ...
                  <div className='tools-content-group-name'>比例</div>
                </div>
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                    <div
                      className={`tools-content-${
                        clicked === 'drag' ? 'clicked' : 'clickeable'
                      }`}
                    ...
                    <div
                      className={`tools-content-${
                        clicked === 'edit' ? 'clicked' : 'clickeable'
                      }`}
                  ...
                  <div className='tools-content-group-name'>模式</div>
                </div>
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                    <div
                    ...
                    >
                      <Icon type='fa-file-image-o' />
                      导出图片
                    </div>
                  </div>
                  <div className='tools-content-group-name'>导出</div>
                </div>
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                    <div
                  ...
                  <div className='tools-content-group-name'>搜索</div>
                </div>
              </div>
              <div
                className='tools-content-tab'
                style={{ display: tools === 'plug' ? '' : 'none' }}
              >
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                    <div
                  ...
                  <div className='tools-content-group-name'>解析导入</div>
                </div>
                <div className='tools-content-group'>
                  <div className='tools-content-group-content'>
                    <div
        ...
        >
          ...
          >
            <div className='tools-left-tab-header'>
              ...
              <div
                className='tools-left-tab-header-tab-names'
                style={{ display: width === 0 ? 'none' : '' }}
              >
                <span
                  className={`${
                    tab === 'table' ? 'menu-tab-tools-edit-active' : ''
                  }`}
                  onClick={() => this._leftTabChange('table')}
                >
                  <Icon type='fa-th' style={{ marginRight: 5 }} />
                  数据表
                </span>
                <span
                  className={`${
                    tab === 'domain' ? 'menu-tab-tools-edit-active' : ''
                  }`}
                  onClick={() => this._leftTabChange('domain')}
                >
                  <Icon type='fa-th-list' style={{ marginRight: 5 }} />
                  数据域
                </span>
              </div>
            </div>
            <div
              className='tools-left-tab-body'
              style={{ display: width === 0 ? 'none' : '' }}
            >
              <div
                className='tools-left-tab-body-table'
                ...
                {(dataSource.modules || []).length > 0 ? (
                  ...
                  >
                    {(dataSource.modules || []).map((module) => {
                      const realName = module.chnname
                        ? `${module.name}-${module.chnname}`
                        : module.name;
                      return (
                        <TreeNode
                          draggable
                        ...
                        >
                          ...
                          >
                            {(module.entities || []).map((entity) => {
                              const realName =
                                this._getTableNameByNameTemplate(entity);
                              return (
                                <TreeNode
                                  realName={realName}
                                  key={entity.title}
              ...
              <div
                className='tools-left-tab-body-domain'
                      ...
                      >
                        {(
                          ...
                          <TreeNode
                            ...
                            realName={`${data.name || data.code}${
                              data.defaultDatabase ? '(默认)' : ''
                            }`}
                            name={
                              <span>
                                <Icon
          ...
          <div className='tools-left-border' onMouseDown={this._onMouseDown}>
            {}
          </div>
          <div
          ...
          >
            {tabs.filter((lefttab) => !lefttab.folding).length === 0 ? (
              ...
              >
                {tabs
                  .filter((lefttab) => !lefttab.folding)
                  .map((leftTab) => {
                    return (
                      <TabPane
