...
const generateIncreaseSql = (
  ...
  const database = _object
    .get(dataSource, 'dataTypeDomains.database', [])
    .filter((db) => db.code === code)[0];
  const template = templateShow
    ? (database && database[templateShow]) || ''
...
const generateUpdateSql = (
  ...
  templateResult += changes
    ...
    .map((c) => {
      if (c.opt === 'update') {
        ...
        const dataTable =
          tempEntities.filter((t) => t.title === change[0])[0] || {};
        const field =
          (dataTable.fields || []).filter((f) => f.name === change[1])[0] || {};
        ...
        const position = (dataTable.fields || []).findIndex(
          (f) => field.name === f.name
        );
        if (position > 0) {
          addAfter =
            ((dataTable.fields || [])[position - 1] &&
              (dataTable.fields || [])[position - 1].name) ||
            undefined;
        }
  ...
  templateResult += changes
    ...
    .map((c) => {
      ...
      const index = indexData.filter((i) => i.name === indexName)[0] || {
        name: indexName,
      };
      if (c.opt === 'add') {
        // 根据数据表中的内容获取索引
        ...
        // 1.先删除再重建
        let deleteString = getTemplateString(
          getTemplate('deleteIndexTemplate'),
          {
            module: { name: dataTable.name },
            entity: dataTable,
            index,
        ...
        let createString = getTemplateString(
          getTemplate('createTableTemplate'),
          {
            module: { name: dataTable.name },
            entity: dataTable,
            index,
  ...
  templateResult += changes
    ...
    .map((c) => {
      if (c.opt === 'add') {
        const change = c.name;
        const dataTable =
          tempEntities.filter((t) => t.title === change)[0] || {};
        ...
        const dataTable =
          tempEntities.filter((t) => t.title === change)[0] || {};
        const oldDataTable =
          oldEntities.filter((t) => t.title === change)[0] || {};
    ...
    .join('\n');
  return templateResult.endsWith(separator)
    ? templateResult
    : templateResult + separator;
};

const getCodeByRebuildTableTemplate = (
  dataSource,
  changes,
  code,
  oldDataSource
...
export const getCodeByDataTable = (
  dataSource,
  module,
  dataTable,
  code,
  ...
  try {
    ...
    if (
      ...
      sqlString = getCodeByRebuildTableTemplate(
        dataSource,
        changes,
        code,
        oldDataSource
  ...
  sqlString += tempEntities
    .map((e) => {
      ...
      allData.createIndex = (e.indexs || [])
        ...
        .join('\n');
      allData.deleteTable = `${getTemplateString(
        getTemplate('deleteTableTemplate'),
        {
          module: { name: e.name },
          entity: e,
          separator,
      ...
      allData.createTable = `${getTemplateString(
        getTemplate('createTableTemplate'),
        {
          module: { name: e.name },
          entity: e,
      ...
      allData.updateComment = `${getTemplateString(
        getTemplate('updateTableComment'),
        {
          module: { name: e.name },
          entity: e,
          separator,
