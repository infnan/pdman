...
const initColumnOrder = (dataTable, columnOrder) => {
  ...
  return headers.filter(
    (header) => !header.relationNoShow && header.fieldName !== 'relationNoShow'
  );
};
...
const getData = (dataSource, moduleName, columnOrder) => {
  ...
  return {
    ...
    nodes: [...(data.nodes || [])].map((node) => {
      const dataTable = getAllTableData(dataSource).filter(
        (entity) => entity.title === (node.copy || node.title.split(':')[0])
      )[0];
      return {
        ...node,
    ...
    draw(cfg, group) {
      ...
      const getTitleColor = (field) => {
        if (field.pk) {
          ...
          currentFromEntities.some((entity) => entity.from.field === field.name)
        ) {
          return '#8F9C6D';
        }
        return '#C5C6C5';
      ...
      height =
        Math.max(
          ...Object.keys(headerBox).map((header) => headerBox[header].height)
        ) +
        4 * padding +
        titleBox.height;
      // 获取每一行的高度
